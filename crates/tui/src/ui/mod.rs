...
use crate::strings::{INPUT_HINT, PREFIX_ASSISTANT, PREFIX_USER};
...
pub fn draw(f: &mut Frame, app: &mut App) {
    if app.show_sidebar {
        ...
        {
            let app_ref: &App = &*app;
            draw_sidebar(f, chunks[0], app_ref);
        }
...
fn draw_status(f: &mut Frame, area: Rect, app: &App, _input_visible_lines: u16, inner_width: u16) {
    ...
    let search_info = if let Some(q) = &app.search_query {
        let total = app.search_hits.len();
        if total > 0 {
            format!("  🔎 {}  ({}/{})", q, app.search_current + 1, total)
        } else {
            format!("  🔎 {} (0/0)", q)
        }
    } else {
    ...
    let tips = format!(
        "[{}][{}] L{} C{}  •  Enter send / Shift+Enter newline  •  PgUp/PgDn scroll  •  Shift+PgUp/PgDn fast  •  Ctrl+↑/↓ fine  •  F2 sessions  •  History ↑/↓ ({})  •  Ctrl+F search / F3 next  •  ? help{}",
        stick, focus, line_disp, col_disp, app.history.len(), search_info
    );
...
fn draw_help(f: &mut Frame, area: Rect) {
    ...
    let lines = vec![
        ...
        Line::from("  ←/→: Cursor move    Backspace/Delete: Delete prev/next char"),
        Line::from("  Home/End: Line start/end    Ctrl+A/E: Line start/end"),
        Line::from("  Ctrl+←/→: Word move    Ctrl+W: Delete prev word"),
        Line::from("  Ctrl+U/K: Kill to line start/end"),
        Line::from("Chat Scrolling"),
        Line::from("  Mouse wheel: Scroll    PgUp/PgDn: Page    Shift+PgUp/PgDn: Fast page    Ctrl+↑/↓: Fine scroll    Click indicator: Expand/collapse"),
        Line::from("  Ctrl+Home/End: Top/bottom    Stick to bottom: Auto when at bottom"),
        Line::from("Sessions & Others"),
        Line::from("  F2: Show/hide sessions    ↑/↓: Input history    Mouse click sidebar: Switch session"),
